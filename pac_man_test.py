...
#-----------------------------------------------------------------------------------
    ...
    def make_minimax_cpu_move(self, t, playerColor, oppColor):
        ...
        #CPU player is always try to maximize the evaluatioin 
        move, score = self.maximize(self, t, alpha, beta, playerColor, oppColor)
    ...
    def maximize(self, board, t, alpha, beta, playerColor, oppColor):
        score_max = float("-inf") 
        move_max = None
       ...
       #possible_moves = get_moves(board, playerColor, oppColor)
        ...
        #every time call, go one level deeper
        ...
        for move in possible_moves:
            ...
            # call the minimize, swap the players
                step, score = self.minimize(temp_board, t, alpha, beta, oppColor, playerColor) 
    ...
    def minimize(self, board, t, alpha, beta, playerColor, oppColor):
        score_min = float("inf") 
        move_min = None
        possible_moves = get_moves(board, playerColor, oppColor)
        choices = len(possible_moves) 
        ...
        for move in possible_moves:
            ...
            if (has_move(temp_board, oppColor, playerColor) == False) and (has_move(temp_board,playerColor, oppColor) == False):
                ...
                step, score = self.maximize(temp_board, t, alpha, beta, oppColor, playerColor) 
...
def main():
    ...
    while (onPlay == -1):
        ...
        try:
            if (player == '1'):
                Game = pac_man_test('W', 'B')
                play(Game)
                onPlay = 1
            elif (player == '2'):
                Game = pac_man_test('B', 'W')
                play(Game)
                onPlay = 1
            else:
